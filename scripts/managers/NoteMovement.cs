...
public class SingleNoteMovementManager {
  ...
  public static Vector3[] RANDOM_ROTATIONS = {
	new Vector3(-0.954387F, -0.1183784F, 0.2741019F),
  ...
  public PositionData updatePosition(Frame frame, Vector3 curPos, Quaternion curRot) {
	...
	// GD.Print(endRotation);
	// GD.Print(movementData.endRotation);
	...
	Vector3 headPos = new Vector3(frame.head.position.X, frame.head.position.Y, frame.head.position.Z);
	...
	if (jumpPercent < 0.5) {
	  if (jumpPercent >= 0.125) {
		...
		// rot = midRotation.Slerp(endRotation, (float)Math.Sin((jumpPercent - 0.125) * Math.PI * 2)); //not the same as sabersim?
		// rot = midRotation.Slerp(endRotation, (float)Math.Sin(Math.PI/2)); //not the same as sabersim?
	  } else {
  ...
  public class MovementData {
	...
	public MovementData(MapInfo.DifficultyBeatmap difficultyBeatmap, BeatMap.Object note) {
	  ...
	  moveEnd = CENTER + Vector3.Back * (jumpDistance * 0.5F); // "start" of note jump, in time with spawn time
	  jumpEnd = CENTER - Vector3.Back * (jumpDistance * 0.5F); // end of jump
	  moveStart = CENTER + Vector3.Back * (moveDistance + jumpDistance * 0.5F); // "spawning animation" start
